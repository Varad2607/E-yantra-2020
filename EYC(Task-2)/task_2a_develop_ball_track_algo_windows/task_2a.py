...
def init_remote_api_server():
        ...
        # This function closes any opened communications/connections
        sim.simxFinish(-1)

        client_id = sim.simxStart('127.0.0.1',19997,True,True,5000,200)
...
def get_vision_sensor_image():
        ...
        if(client_id != -1):
                ...
                if(len(vision_sensor_image) != 0):
                        average = sum(vision_sensor_image) / len(vision_sensor_image)

                #i = 0
                 
                while(sim.simxGetConnectionId(client_id)!=-1 and len(image_resolution) != 2 and len(vision_sensor_image) <= 0 and average ==0):
                        #i = i + 1
                        ...
                        #print(sim.simx_return_ok)
                                ...
                                #return_code=sim.simxSetVisionSensorImage(client_id,handle,vision_sensor_image,0,sim.simx_opmode_oneshot)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
        ...
        NumpyArray = np.asarray(vision_sensor_image,dtype = 'uint8')
        #print(NumpyArray.shape)
        #print(NumpyArray)

        #ThreedyArray = NumpyArray.reshape(-1,1280,3,order='F').swapaxes(0,1)
        ThreedyArray = NumpyArray.reshape(1024,1024,3)
        #print(ThreedyArray)

        #ThreedyArray = np.reshape(NumpyArray, (NumpyArray.shape[0]//3, 3))
        #print(ThreedyArray)
        

        image = cv2.cvtColor(ThreedyArray, cv2.COLOR_BGR2RGB)
        #print(image)
...
if __name__ == "__main__":
        ...
        try:
                ...
                #cv2.imshow("Img",vision_sensor_image)
